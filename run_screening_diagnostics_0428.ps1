...
$OutputBase = Join-Path $RootDir "rerun_0428_outputs\screening_diagnostics_0428"
...
$steps += [pscustomobject]@{
    id = "01_diagnostics";
    label = "screening_diagnostics_0428";
    status = "pending";
    output_dir = $OutputRoot;
    log_file = (Join-Path $LogDir "01_diagnostics.log");
    started_at = $null;
    ended_at = $null;
}
...
$pythonArgs = @(
    (Join-Path $RootDir "screening_diagnostics_0428_experiment.py"),
    "--input", $InputFile,
    "--output-dir", $OutputBase
) + $extraArgs

Invoke-DiagnosticsStep -StepId "01_diagnostics" -Label "screening_diagnostics_0428" -PythonArgs $pythonArgs

Write-Host "Screening diagnostics experiment completed. Output: $OutputRoot"
