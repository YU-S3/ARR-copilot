...
$OutputRoot = Join-Path $RootDir "rerun_0428_outputs\screening_tuning_0428\$Mode"
$ScriptOutputRoot = Join-Path $RootDir "rerun_0428_outputs\screening_tuning_0428"
...
if ($LASTEXITCODE -ne 0 -or -not $pythonPath) {
    throw "Cannot run python. Please run this from an activated arr_rf PowerShell prompt or install conda."
}
...
$steps += [pscustomobject]@{
    id = "01_tuning";
    label = "screening_tuning_0428";
    status = "pending";
    output_dir = $OutputRoot;
    log_file = (Join-Path $LogDir "01_tuning.log");
    started_at = $null;
    ended_at = $null;
}
...
$pythonArgs = @(
    (Join-Path $RootDir "screening_tuning_0428_experiment.py"),
    "--input", $InputFile,
    "--output-dir", $ScriptOutputRoot
) + $extraArgs

Invoke-TuningStep -StepId "01_tuning" -Label "screening_tuning_0428" -PythonArgs $pythonArgs

Write-Host "Screening tuning completed. Output: $OutputRoot"
